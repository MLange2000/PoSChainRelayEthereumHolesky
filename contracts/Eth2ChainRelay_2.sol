...
// PLEASE NOTE: This smart contract version was used in the development process for unit testing 
//              the helper functions and is not up to date with the other versions.
...
uint constant SYNC_COMMITTEE_SIZE = 2;
...
contract Eth2ChainRelay_2 {
    ...
    function submitUpdate(ChainRelayUpdate memory _chainRelayUpdate, SyncCommitteeUpdate memory _syncCommitteeUpdate) public returns (bool) {
        ...
        //require(
            validateMerkleBranch(
                ...
                SLOT_INDEX, 
                _chainRelayUpdate.latestSlotBranch)
                //,"merkle proof for latest slot not valid")
            ;
        /*require(
            validateMerkleBranch(
                _chainRelayUpdate.stateRoot, 
        ...
        //require(
            validateMerkleBranch(
                ...
                SLOT_INDEX, 
                _chainRelayUpdate.finalizedSlotBranch)
                //, "merkle proof for finalized slot not valid")
            ;
        require(
            validateMerkleBranch(
                _chainRelayUpdate.finalizedBlockRoot, 
    ...
    function merkleize(bytes32[SYNC_COMMITTEE_SIZE] memory _chunks) public pure returns (bytes32) {
        ...
        for (uint i = _chunks.length; i < length_next_pow; i++) {
           // fill lowest level 
           // TODO not necessary b/c size always 2^n?
        }
